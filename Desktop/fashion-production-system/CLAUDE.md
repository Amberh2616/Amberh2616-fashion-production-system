...
|------|------|
| **本文件** | 核心參考（指令、服務、架構）|
| **[docs/SDD.md](docs/SDD.md)** | 軟體設計規格（Software Design Document）|
| **[docs/TDD.md](docs/TDD.md)** | 技術設計文檔（Technical Design Document）|
...
**需求：**
比較報價與實際成本，計算真正的 COST DOWN / 盈虧。

**核心公式：**
```
毛利 = 報價金額 - 實際成本
毛利率 = (報價金額 - 實際成本) / 報價金額 × 100%
```
...
| 項目 | 報價階段（預估）| 結案階段（實際）|
|------|----------------|----------------|
| 物料成本 | BOMItem 單價 × 用量 | PO 實際金額 |
| 人工成本 | Operation 工時 × 單價 | 實際工時記錄 |
| 損耗 | 預設 % | 實際損耗記錄 |
| **總成本** | CostSheet.total_cost | 實際總成本 |
...
│ 成本分析報表（100 款）                                        │
├─────────────────────────────────────────────────────────────┤
│ 款號    │ 報價    │ 實際成本 │ 毛利   │ 毛利率 │ 狀態     │
│ LW1ABC  │ $50.00  │ $42.00  │ $8.00  │ 16%   │ ✅ 盈利  │
│ LW1DEF  │ $35.00  │ $38.00  │ -$3.00 │ -8.6% │ ❌ 虧損  │
│ ...     │         │         │        │       │          │
├─────────────────────────────────────────────────────────────┤
│ 總計    │ $5,000  │ $4,200  │ $800   │ 16%   │          │
└─────────────────────────────────────────────────────────────┘
```
